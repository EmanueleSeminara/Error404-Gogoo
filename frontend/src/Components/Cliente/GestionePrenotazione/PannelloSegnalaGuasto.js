    ...
    Axios.put("/api/guest/damagedvehicle", data)
      ...
      .catch((err) => {
        ...
        if (err.response.status === 422) {
          ...
          this.setState({
            string:
              "impossibile segnalare il guasto al momento, riprova più tardi",
          });
          this.setState({ error: true });
        } else {
          window.location.href = "/errorServer";
  ...
  render() {
    return (
      <div className="ez sfondo-card">
        ...
        {this.state.success && (
          <Alert severity="success"> segnalazione avvenuta con successo</Alert>
        )}
        <div className="row justify-content-md-center  ">
          <div className="d-flex flex-column pannell-User ">
            <center>
              <div className="title">Segnala Guasto</div>
            </center>
            ...
            {this.state.listReservation.map((item) => (
              <div className="p-3 col-12">
                <CardSegnalaGuasto
                  ...
                  state={item.state}
                  segnaleGuasto={this.segnaleGuasto}
                />
              </div>
            ))}
          </div>
