...
exports.myReservation = (id) => {
    return new Promise((resolve, reject) => {
        const sql = "SELECT r.id,r.refVehicle, v.type, v.category, r.dateR, r.dateC, r.refParkingR, r.refParkingC, r.refDriver, r.positionR, r.positionC, r.state, u.name, u.surname FROM reservations AS r JOIN vehicles AS v ON r.refVehicle= v.id JOIN users AS u ON u.id = r.refGuest WHERE r.refDriver = ?"
...
exports.reservationNotConfirmed = () => {
    return new Promise((resolve, reject) => {
        const sql = "SELECT r.id,r.refVehicle, v.type, v.category, r.dateR, r.dateC, r.refParkingR, r.refParkingC, r.refDriver, r.positionR, r.positionC, r.state, u.name, u.surname FROM reservations AS r JOIN vehicles AS v ON r.refVehicle= v.id JOIN users AS u ON u.id = r.refGuest WHERE r.state = 'not  confirmed'"
        db.all(sql, [id], (err, rows) => {
            if (err) {
              reject(err);
              return;
